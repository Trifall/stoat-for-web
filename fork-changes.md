...
Alternate `/i/:host` routes currently redirect to the default instance before loading an alternate configuration. This is intentional until instance-specific persistence and session isolation are designed, so the route structure must not be described as completed multi-instance support. Backend links such as webhook URLs must use `Instance.apiUrl`.

Upstream #1521 (merged 2026-08-21) replaced the fork's earlier `location.origin`-based `Instance.href()` with canonical link generation from `this.origin` (`https://<host || DefaultHost>`): relative mode returns a full origin-rooted URL with no `/i/` prefix, and `absPath` mode returns `/i/<base>`-prefixed paths for internal navigation. Do not restore the old semantics. The desktop app serves the UI from the custom `stoat://` protocol, under which the old absolute links were rejected by `RenderAnchor`'s http/https protocol whitelist (dead channel mentions/system-message anchors) and produced unshareable clipboard URLs; canonical https links pass that check, match `inAppScope`, and are rebuilt into router `<Link>` navigations against `location.origin`, keeping SPA routing inside `stoat://`. In-app internal navigation still depends on the shared `Redirect` guard prefixing `/i/<host>`, so keep `href()` and `Redirect` semantics together during future upstream refactors.
...
Upstream's `VoiceLayout` state is integrated with the fork call UI: `collapsed` renders the compact control bar, `expanded` maximizes within the measured channel-content parent, and `fullscreen` uses the browser fullscreen path. Keep these modes mutually exclusive through `voice.layout()`; do not restore the fork's former separate `maximized` and `fullscreen` signals. Expanded mode must remain constrained to the channel content so it does not cover the left navigation.
