...
export default class Instance {
  ...
  _init() {
    const [limits, setLimits] = createSignal(this.baseLimits);
    ...
    createEffect(() => {
      ...
      const updateLimits = () => setLimits(client.limits ?? this.baseLimits);

      updateLimits();
      client.on("ready", updateLimits);
      onCleanup(() => client.off("ready", updateLimits));
    });
  }
